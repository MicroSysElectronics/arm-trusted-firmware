...
static void mmap_add_ddr_regions_statically(void)
{
	...
	if (info_dram_regions->region[i].size >
		...
		VERBOSE("S.Secure DRAM Region %d: %p - %p\n", i,
			(void *) (info_dram_regions->region[i].addr
				+ info_dram_regions->region[i].size),
			(void *) (info_dram_regions->region[i].addr
				+ info_dram_regions->region[i].size
...
#ifdef IMAGE_BL31
	VERBOSE("mmap_add_ddr_regions_statically: info_dram_regions->num_dram_regions = %lld\n",
		info_dram_regions->num_dram_regions);
	for (i = 1; i < info_dram_regions->num_dram_regions; i++) {
		VERBOSE(" info_dram_regions->region[%d].size = %llx\n",
			i,info_dram_regions->region[i].size);
...
void mmap_add_ddr_region_dynamically(void)
{
	...
	if (info_dram_regions->region[i].size >
		(NXP_SECURE_DRAM_SIZE + NXP_SP_SHRD_DRAM_SIZE)) {
		VERBOSE("D.Secure DRAM Region %d: %p - %p\n", i,
			(void *) (info_dram_regions->region[i].addr
				+ info_dram_regions->region[i].size),
			(void *) (info_dram_regions->region[i].addr
				+ info_dram_regions->region[i].size
...
#ifdef IMAGE_BL31
	VERBOSE("mmap_add_ddr_region_dynamically: info_dram_regions->num_dram_regions = %lld\n",
		info_dram_regions->num_dram_regions);
	for (i = 1; i < info_dram_regions->num_dram_regions; i++) {
		VERBOSE(" info_dram_regions->region[%d].size = %llx\n",
			i,info_dram_regions->region[i].size);
