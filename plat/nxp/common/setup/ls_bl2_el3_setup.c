...
static void populate_dram_regions_info(void)
{
	...
	if (dram_remain_size > 0) {
		reg_id++;
		dram_regions_info.region[reg_id].addr = NXP_DRAM2_ADDR;
		dram_regions_info.region[reg_id].size =
				dram_remain_size > NXP_DRAM2_MAX_SIZE ?
					NXP_DRAM2_MAX_SIZE : dram_remain_size;
		dram_remain_size -= dram_regions_info.region[reg_id].size;
	}
#endif
	reg_id++;
...
void bl2_plat_preload_setup(void)
{
	...
	INFO(" dram_regions_info.region[0].addr %llx .size %llx dram_regions_info.total_dram_size %lld\n",
       dram_regions_info.region[0].addr, dram_regions_info.region[0].size, dram_regions_info.total_dram_size);
	INFO(" dram_regions_info.region[1].addr %llx .size %llx dram_regions_info.total_dram_size %lld\n",
       dram_regions_info.region[1].addr, dram_regions_info.region[1].size, dram_regions_info.total_dram_size);
