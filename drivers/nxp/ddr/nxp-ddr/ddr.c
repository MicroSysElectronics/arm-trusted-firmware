...

{
	...
	if (pdodt == NULL) {
		ERROR("Error determing ODT. Using dual_DD\n");
		pdodt = dual_DD;
		// return -EINVAL;
	}
...
static int parse_spd(struct ddr_info *priv)
{
...
#ifdef CONFIG_TARGET_MPXLX2160
	/* Read the SPD on the SODIMM.
	 * If it can be read: a SODIMM inserted
	 * if not: no SODIMM inserted
	 */
	struct ddr4_spd spd[2];
	unsigned int spd_checksum[2];
	int spd_idx = 0;
	int ret, addr, x = 0;
	struct dimm_params *sodimm = &priv->sodimm;

	memset(sodimm, 0, sizeof(struct dimm_params));
	...
	if (ret) {	/* invalid */
		INFO("No SODIMM at address 0x%x\n", addr);
	} else {
		// SODIMM found. Calculate the parameters and check via fixed setup

		// dump_spd(&spd[spd_idx], sizeof(struct ddr4_spd));
		...
		INFO("SPD checksum 0x%x\n", spd_checksum[spd_idx]);
		if (!spd_checksum[spd_idx]) {
			printf("Bad checksum, ignored.\n");
		}
		/* first valid SPD */
		ret = cal_dimm_params(&spd[0], sodimm);
		if (ret) {
			ERROR("SPD calculation error\n");
			return -EINVAL;
		}
		// Set the number of CS based on the SPD
		conf->dimm_in_use[1] = sodimm->n_ranks;

		NOTICE("Differences between soldered DIMM and SODIMM:\n");
		// Check for differences between Timing
		if (sodimm->edc_config > dimm->edc_config) {
			NOTICE(" DIMM without ECC\n");
			dimm->edc_config = 0;
			sodimm->edc_config = 0;
			x++;
		}
		if (dimm->edc_config > sodimm->edc_config) {
			NOTICE(" SODIMM without ECC\n");
			dimm->edc_config = 0;
			x++;
		}
		LESS(tckmin_x_ps);
		...
		LESS(trrdl_ps);
		LESS(tccdl_ps);
		MORE(refresh_rate_ps);

		if (x == 0) {
			NOTICE(" no timing differences\n");
		}
	}
#endif
	...
	unsigned long long current_mem_base)
{
	...
	debug("Total Mem %lldGB\n",total_ctlr_mem/(1024*1024*1024));
